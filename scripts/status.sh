...
db_path = root / 'data/candidaturas.db'
...
print(f"Score mínimo: {config['min_fit_score']} | meta mínima diária: {config['daily_target_min']} | sem teto diário")
print(f"Notificações: {config['notification_target']}")
...
print(f"Notificações Discord pendentes: {conn.execute('SELECT COUNT(*) FROM notifications WHERE delivered_at IS NULL').fetchone()[0]}")
